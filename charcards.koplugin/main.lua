...
local L = {
    ...
    prompt_update_character = 'Ось цитата з книги:\n"""\n{{quote}}\n"""\n\n' ..
        ...
        "словами, без дублювання ЗМІСТУ. Те саме для \"relationships\" — поверни ПОВНИЙ список " ..
        "усіх звʼязків персонажа (уже відомі + нові з цитати), об'єднавши схожі за змістом в один " ..
        "запис (наприклад, \"донька короля\" і \"донька колишнього короля\" — це той самий " ..
        "звʼязок, лиши один, точніше формулювання). Якщо цитата підказує те саме, що вже записано, лише " ..
        ...
        '  "relationships": ["ПОВНИЙ список звʼязків (старі+нові, об\'єднані за змістом, без дублів) — кожен завжди у формі \'цей персонаж є [хтось] відносно [когось]\' (напр. \'донька короля\', не \'батько — король\')"],\n' ..
...
local function mergeUpdateIntoCard(card, update)
    ...
    -- Звʼязки — той самий принцип, що й текстові поля: Gemini бачить
    -- ПОВНИЙ поточний список і повертає вже об'єднаний, дедуплікований ЗА
    -- ЗМІСТОМ список (не лише за буквальним текстом) — тому просто
    -- замінюємо весь масив, а не дописуємо по одному з дедуплікацією по
    -- точному рядку (те й давало дублі типу "донька короля" /
    -- "донька колишнього короля" як два різні записи).
    if type(update.relationships) == "table" and #update.relationships > 0 then
        local cleaned = {}
        for _, r in ipairs(update.relationships) do
            r = trim(r)
            if r ~= "" then table.insert(cleaned, r) end
        end
        if #cleaned > 0 then
            local old_joined = table.concat(card.relationships or {}, "|")
            local new_joined = table.concat(cleaned, "|")
            if new_joined ~= old_joined then
                card.relationships = cleaned
                table.insert(changed, "звʼязки")
            end
        end
    end
